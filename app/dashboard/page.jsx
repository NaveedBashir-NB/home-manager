...
export default function DashboardPage() {
  ...
  const markCompleted = async (id) => {
    ...
    await fetch(`/api/items/${id}`, {
      ...
      body: JSON.stringify({ email: user.email, item: updatedItem }),
    });
    ...
    <div className="px-6 py-10 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-10">
        {[
          ...
          <div key={box.label} onClick={box.action} className="cursor-pointer bg-[var(--card-bg)] border border-[var(--dropdown-border)] rounded-xl p-6 text-center hover:scale-105 transition shadow-md select-none">
      ...
      <div className="bg-[var(--card-bg)] p-5 rounded-xl border border-[var(--dropdown-border)] shadow mb-10">
        {filteredItems.length === 0 ? (
          ...
          <ul className="space-y-3">
            {Array.isArray(filteredItems) && filteredItems.slice(0, 10).map(item => (
              <li key={item._id} className="flex justify-between items-center p-3 rounded-lg bg-black/10 dark:bg-white/10">
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-xs font-semibold px-5 py-2 rounded-full ${
                  ...
                  <button onClick={() => goEdit(item._id)} title="Edit" className="text-[var(--text-main)] hover:text-yellow-400 transition">✏️</button>

                  {item.status !== "completed" && (
                    <button onClick={() => markCompleted(item._id)} title="Mark completed" className="text-green-400 hover:text-green-300 transition">✔</button>
                  )}

                  <button onClick={() => deleteItem(item._id)} title="Delete" className="text-red-400 hover:text-red-300 transition">🗑️</button>
                </div>
              </li>
            ))}
          </ul>
